...
- Before publishing to `main`, identify the current marketplace submission/update request in `docs/publishing.md` and run `python3 -B tools/check_marketplace_review.py` from the development checkout. While review is pending, freeze `main`, including documentation-only commits. Record post-submission publication/test evidence in the release or submission, not a new `main` commit. Necessary review fixes require updating the submission to the new full SHA and rerunning both marketplace reports. Follow `docs/publishing.md`; a passing drift check does not grant approval or permission to push.

- On release approval or a release-status question, compare current remote `main`, GitHub releases, the latest marketplace update request and `development` before answering. Follow the post-approval handoff in `docs/publishing.md`; report pending publication or unreleased changes explicitly and complete already-authorized release work.
