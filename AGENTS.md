...
- Before publishing to `main`, check marketplace review #4530 and run `python3 -B tools/check_marketplace_review.py` from the development checkout. While review is pending, freeze `main`, including documentation-only commits. Record post-submission publication/test evidence in the release or submission, not a new `main` commit. Necessary review fixes require updating the submission to the new full SHA and rerunning both marketplace reports. Follow `docs/publishing.md`; a passing drift check does not grant approval or permission to push.
